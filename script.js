...
function updateDom(data) {
    ...
    data.map(item => {
        ...
        <tr>
          ...
          <td colspan="2"  class="img"> <img src="${item.img_src}"> ${item.first_name} ${item.last_name}</td>
          <td>${item.gender}</td>
          <td>${item.class}</td>
          <td>${item.marks}</td>
          <td>${pass}</td>
...
sortA.addEventListener('click', () => {
    let array = sort_array(arr, 'first_name')
    updateDom(array);

})

function sort_array(arr, sort) {
...
sortMarks.addEventListener('click', () => {
    let array = sort_array(arr, 'marks')
    updateDom(array);
})

sortClass.addEventListener("click", () => {
    let array = sort_array(arr, 'class')
    updateDom(array);
})

sortZ.addEventListener('click', () => {
    let array = sort_arrayZ(arr, 'first_name')
    updateDom(array);
})

function sort_arrayZ(arr, sort) {
